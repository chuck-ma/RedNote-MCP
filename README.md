...
- 使用工具 `search_notes`，参数：`keywords`（必填）、`limit`（可选，默认 10）
- 搜索流程会先进入首页，等待搜索框出现后模拟人类输入并按下回车，再解析搜索结果
- 内置 3 次失败重试（会在重试前清理浏览器），对小红书前端样式变化做了多种选择器兼容
- 搜索阶段包含随机停顿与真实浏览器操作，整体耗时会比直接请求接口更长
- 如果返回为空或提示未登录，请先运行 `rednote-mcp init` 重新获取可用的登录态
